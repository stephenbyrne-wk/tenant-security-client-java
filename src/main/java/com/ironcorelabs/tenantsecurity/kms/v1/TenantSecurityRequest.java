...
final class TenantSecurityRequest implements Closeable {
   ...
   * callers. Attempts to parse the response as JSON and convert the received error code over to the
  ...
  CompletableFuture<WrappedDocumentKey> wrapKey(DocumentMetadata metadata) {
    ...
    String error = String.format(
        ...
        this.wrapEndpoint);
    return this.makeRequestAndParseFailure(this.wrapEndpoint, postData, WrappedDocumentKey.class,
        error);
  }
  ...
  /**
   * Request batch unwrap endpoint with the provided map of edeks. Returns a map of EDEK key to DEK
   * for successes and a map of EDEK key to failure details for failures.
   */
  ...
  /**
   * Request re-key endpoint to unwrap an EDEK encrypted to the metadata's tenantId and then wrap it
   * to the newTenantId.
   */
  CompletableFuture<RekeyedDocumentKey> rekey(String edek, DocumentMetadata metadata,
      String newTenantId) {
    Map<String, Object> postData = metadata.getAsPostData();
    postData.put("encryptedDocumentKey", edek);
    ...
    String error = String.format(
        ...
        this.rekeyEndpoint);
    return this.makeRequestAndParseFailure(this.rekeyEndpoint, postData, RekeyedDocumentKey.class,
        error);
  }
  ...
  /**
   * Create the factory for http requests. The main reason for this is to provide connection pooling
   * for situations where large numbers of requests are desired.
   *
